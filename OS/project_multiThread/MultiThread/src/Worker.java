...
public class Worker extends Thread {
   ...
   public final AtomicInteger runningCount; //นับงานที่กำลังทำอยู่ทั้งระบบ
    public Worker(String name, ReadyQueue readyQueue, ResourceManager resources,
                  Statistics statistics, ProjectLogger logger, AtomicInteger runningCount)throws UnsupportedOperationException {
        super(name);
        // TODO
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                // เริ่มถืองานนี้ที่กำลังทำอยู่ ตั้งแต่รับมาจนจบ
                runningCount.incrementAndGet();

                try{
                    processJob(job);
                }finally{
                    // ต้อง decrement เสมอ แม้ processJob throw exception หรือ interrupt กลางคัน
                    // ไม่งั้นตัวนับ running จะค้าง ไม่มีวันลดลง และ Monitor จะรายงานผิด
