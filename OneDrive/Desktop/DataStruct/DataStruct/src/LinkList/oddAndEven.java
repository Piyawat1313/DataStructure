...
public class oddAndEven {
    public static void main(String[] args) {
        try {
            ...
            QueseLinkList Odd = new QueseLinkList();
            QueseLinkList Even = new QueseLinkList();
            ...
            for (int i = 0; i < sizeEven; i++) {    //ปริ้นข้อมูลคิวออกมาทั้งหมด
                ...
                Even.enQuese(((int)d));    //เอาไปใส่ไว้ใน คิวต่อ  
            }
            System.out.println();
            ...
            for (int i = 0; i < sizeOdd; i++) {     //ปริ้นข้อมูลคิวออกมาทั้งหมด
                ...
                System.out.print((int)d + " ");
                Odd.enQuese(((int)d));     //เอาไปใส่ไว้ใน คิวต่อ
            }
            System.out.println();
            int countOdd = 0;
            int countEven = 0;
            while (!Even.isEmptyQuese() && !Odd.isEmptyQuese()) {
                ...
                if(frontEven > frontOdd){       //ถ้า Even ชนะ
                    ...
                    Odd.enQuese(((int)Odd.front()));   //เอาข้อมูลของ Odd ตัวหน้าเข้าไปอยู่ในคิวใหม่
                    Odd.deQuese();  //ลบข้อมูลของ Odd ออกไป
                    countEven++;    
                }
                else if(frontOdd > frontEven){      //ถ้า Odd win
                    Odd.deQuese();      //ลบข้อมูลตัวหน้าของ Odd ออกไป
                    Even.enQuese((int)Even.front());     //เอาข้อมูลตัวหน้าของ Even เข้าไปอยู่ในคิว
